...
function computeTotal(
  lineItems: { quantity: number; unitAmount: number }[],
): number {
...
export default async function DashboardPage() {
  ...
  const outstanding = allInvoices
    .filter((inv) =>
      ([InvoiceStatus.SENT, InvoiceStatus.VIEWED] as InvoiceStatus[]).includes(
        inv.status,
      ),
    )
    .reduce((sum, inv) => sum + computeTotal(inv.lineItems), 0);
    ...
    <>
      ...
      <div className="rounded-lg border border-border bg-card shadow-sm">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="p-4 w-32.5">Date</TableHead>
